...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(len(sta)):
            ...
            if val == 0:
                input[i] = self.word_vocab['<ROOT>']
                continue
            if pos[val] == '<CD>':
                input[i] = self.word_vocab['<CD>']
            if pos[val] == '<NNP>':
                input[i] = self.word_vocab['<NNP>']

            word = words[val].lower()
            if word not in self.word_vocab.keys():
                input[i] = self.word_vocab['<UNK>']
            else:
                input[i] = self.word_vocab[word]
        ...
        for i in range(len(buf)):
            ...
            if val == 0:
                input[i+3] = self.word_vocab['<ROOT>']
                continue
            if pos[val] == '<CD>':
                input[i+3] = self.word_vocab['<CD>']
            if pos[val] == '<NNP>':
                input[i+3] = self.word_vocab['<NNP>']

            word = words[val].lower()
            if word not in self.word_vocab.keys():
                input[i+3] = self.word_vocab['<UNK>']
            else:
                input[i+3] = self.word_vocab[word]
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        y = self.output_labels[output_pair]
        return keras.utils.to_categorical(y, num_classes=91, dtype=int)
